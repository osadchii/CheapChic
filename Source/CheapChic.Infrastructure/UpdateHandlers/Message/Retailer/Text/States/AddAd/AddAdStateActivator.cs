...
public interface IAddAdStateActivator : IRetailerStateActivator
{
}

public class AddAdStateActivator : IAddAdStateActivator
{
    private readonly ITelegramBot _telegramBot;
    private readonly IUserService _userService;

    public AddAdStateActivator(ITelegramBot telegramBot, IUserService userService)
    {
    ...
    {
        var request = SendReplyKeyboardRequest.Create(user.ChatId, MessageText.Retailer.AddAd.SelectAction,
            ConstantMenu.Retailer.AddAnnouncementMenu);

        await _telegramBot.SendReplyKeyboard(bot.Token, request, cancellationToken);
        await _userService.SetUserState(user.Id, bot.Id, State.RetailerAddAdAction, stateData,
            cancellationToken);
    }
}
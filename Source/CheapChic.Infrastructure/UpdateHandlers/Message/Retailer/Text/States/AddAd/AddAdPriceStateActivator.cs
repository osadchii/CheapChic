...
public interface IAddAdPriceStateActivator : IRetailerStateActivator
{
}

public class AddAdPriceStateActivator : IAddAdPriceStateActivator
{
    private readonly ITelegramBot _telegramBot;
    private readonly IUserService _userService;

    public AddAdPriceStateActivator(ITelegramBot telegramBot, IUserService userService)
    {
    ...
    {
        var request = SendReplyKeyboardRequest.Create(user.ChatId, MessageText.Retailer.AddAd.SendPrice,
            ConstantMenu.Retailer.AddAnnouncementPriceMenu);

        await _telegramBot.SendReplyKeyboard(bot.Token, request, cancellationToken);
        await _userService.SetUserState(user.Id, bot.Id, State.RetailerAddAdPrice, stateData,
            cancellationToken);
    }
}